...
router.get("/fetch-cancelled-order", (req, res) => {
    var pendingOrders = `SELECT *, DATE_FORMAT(date_time, "%W %M %e %Y %r") as date_time  FROM orders WHERE order_status = "Cancelled"`;
...
router.get("/cancelled-order", (req, res) => {
    res.render("admin/cancelled_order", {title: "Cancelled Orders", text: "Cancelled Orders"});
});
