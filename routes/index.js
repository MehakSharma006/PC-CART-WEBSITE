...
router.get("/fetch-filtered-product", (req, res) => {
  let { id } = req.query;

  let product = `SELECT * FROM products WHERE subcategory_id=${id}`;
  connection.query(product, (e, rows) => {
    if (e) {
...
router.get("/get-sub-categories", (req, res) => {
  let category =
    "SELECT * FROM `subcategory` ORDER BY `subcategory`.`subcategory_name` ASC";
  connection.query(category, (e, row) => {
    if (e) {
...
router.post("/product-description", (req, res) => {
  console.log(req.body);
  let { id } = req.body;

  let fetchProduct = `SELECT * FROM products WHERE id=${id}`;
  connection.query(fetchProduct, (e, row) => {
    if (e) {
      return res.send(e.message);
    }
    res.send(row);
...
router.get("/shopping-cart", (req, res) => {
  res.render("shopping-cart", {
    title: "Shopping Cart",
    text: "Shopping Cart from public",
  });
});
...
router.get("/get-products-list", (req, res) => {
  ...
  if (req.query.action === "index") {
    products = `SELECT * FROM products ORDER BY id  DESC LIMIT 4`;
  } else {
    products = `SELECT * FROM products ORDER BY id  DESC`;
  }
  connection.query(products, (error, rows) => {
    if (error) {
      res.send("error");
    } else {
      res.send(rows);
...
router.get("/shop", (req, res) => {
  res.render("shop", { title: "Latest Products", text: "Products" });
});

router.get("/contact-us", (req, res) => {
  res.render("contact", { title: "Contact", text: "Contact" });
});

router.get("/about-us", (req, res) => {
  res.render("about", { title: "About", text: "About" });
});
...
// review section
router.post("/reviewaction", function (req, res) {
  console.log(req.body);
...
router.get("/reviewaction", function (req, res) {
  console.log(req.query);
  let email = req.query.e;
  let username = req.query.p;
  let review = req.query.ph;

  let Query =
    "INSERT INTO `reviews`(`email`, `username`, `review`) VALUES ('" +
    email +
    "','" +
    username +
    "','" +
